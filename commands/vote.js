...
module.exports = {
  ...
  async execute(interaction) {
    ...
    let temp = await voteCommandDB(voterId, voteeId);
    let leaders = Object.values(temp);
    let oldLeaderId = leaders[0];
    let newLeaderId = leaders[1];

    if (newLeaderId > 0) {
      let rolePleb = interaction.guild.roles.cache.find(
        (r) => r.id === "1031334547591274556"
      );
      let roleLeader = interaction.guild.roles.cache.find(
        (r) => r.id === "1031329937287807046"
      );
      ...
      const leaderChangeMessage = new EmbedBuilder()
        .setTitle("New Leader!")
        .setDescription(
          `Congratulations, ${newLeader}, you are the new leader!`
        )
        .setColor(0x7df9ff);
